...
function Measure() {
    ...
    const { isMeasureAreaWindowVisible, updateIsMeasureAreaWindowVisible, toggleComponent } = useUtility();
    const [title] = useState('Measure');
    const { backgroundColor, textColor, borderColor, typoColor } = useColor();

    const { olMap, clearVectorSource, stopDrawAction } = useContext(OLMapContext);
